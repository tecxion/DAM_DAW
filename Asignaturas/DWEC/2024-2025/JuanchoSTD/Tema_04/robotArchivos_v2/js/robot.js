...
/*
APOlica unión a todos los elementos (union es conmutativa)
*/
function unionMultiple(...objetos) {
    return objetos.reduce((acumulado, actual) => union(acumulado, actual), {});
}
...
function conteoPalabrasEnlaces(){
    ...
    const conteoPalabras = cuentaPalabras(elementos, ['textContent', 'pathname']); //Ojito, pathname tb y aún no ordeno
...
function escribeAnalisis(){
    ...
    */
    ...
    escribeDerecha("<br><p>Tras hacerlo a mi manera, busco ahora ajustar los factores de forma que nos acerquemos a las palabras que queremos que aparezcan."); 
    ...
    function aplicarCombinaciones(listaCP, combinaciones) {
        ...
        for(let i = 0; i < combinaciones.length; i++) {
            ...
            // Guardamos el resultado junto con los factores usados 
            resultados.push({
                factores: factores,
                resultado: resultado
            });
    ...
    function evaluarResultados(resultadosFinales, listaPalabrasABuscar) {
        ...
        // Ordenamos por valor acumulado de menor a mayor
    ...
    escribeDerecha("<br><br><p>Es fácil encontrar regularidades ahora, por ejemplo Enlaces pesa mucho de manera sostenida y \
         los pies de imágenes no parecen tener mucho valor, pero es para estas palabras específicas y este documento ¿Qué pasará en otro escenario?. \
...
// Lo he tenido que hacer así para poder denotar que el proceso está en marcha, dado que se lleva a veces demasiado tiempo.
async function procesaTodo(){
    document.getElementById('derecha').innerHTML = "<span id='procesando'>Procesando....</span>";

    await new Promise(resolve => setTimeout(resolve, 0)); //Es un modo de refrescar el dom
    escribeDerecha("<h2>Palábras más importantes:</h2>");
