...
switch ($accion) {
    ...
    case 'listar_facturas':
        ...
        if (isset($_REQUEST['XML']) && $_REQUEST['XML'] == '1') {
            ...
            $root = $xml->createElement('facturas');
            $xml->appendChild($root);

            foreach ($facturas as $fila) {
                $factura_node = $xml->createElement('factura');

                $codfra = $xml->createElement('codfra', $fila['codfra']);
                $factura_node->appendChild($codfra);

                $fecha = $xml->createElement('fecha', $fila['fecha']);
                $factura_node->appendChild($fecha);

                $dni_node = $xml->createElement('dni', $fila['dni']);
                $factura_node->appendChild($dni_node);
                
                // Si hay más campos en la tabla cab_factura, añadirlos aquí
                // Ejemplo: $total = $xml->createElement('total', $fila['total']);
                // $factura_node->appendChild($total);
